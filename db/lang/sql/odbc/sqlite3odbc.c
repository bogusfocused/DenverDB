...
InUn(int remove, char *cmdline)
{
...
#else
	static char *drivername = "DenverDB ODBC Driver";
	static char *dsname = "DenverDB Datasource";
#endif
	char *dllname, *p;
	char dllbuf[1000], path[1000], driver[2000], attr[1000], inst[1000];
	char programfiles[1000], proposedpath[1200];
	GetEnvironmentVariable("PROGRAMFILES", programfiles, sizeof(programfiles));
	sprintf(proposedpath, "%s\\Denver DB\\", programfiles);
	if (!remove)
		CreateDirectory(proposedpath, NULL);
	sprintf(proposedpath, "%s\\Denver DB\\ODBC Driver\\", programfiles);
	if (!remove)
		CreateDirectory(proposedpath, NULL);
	...
	//if (SQLInstallDriverManager(path, pathmax, &pathlen)) { This call is deprecated
	sprintf(driver, "%s;Driver=%s%s;Setup=%s%s;",
		drivername, proposedpath, dllname, proposedpath, dllname);
	p = driver;
	while (*p) {
		if (*p == ';') {
			*p = '\0';
	...
	usecnt = 0;
	path[0] = '\0';
	SQLInstallDriverEx(driver, proposedpath, path, pathmax, NULL,
	...
	if (remove) {
		...
		sprintf(attr, "DSN=%s;Database=denverdb.db;", dsname);
		p = attr;
		while (*p) {
			if (*p == ';') {
				*p = '\0';
